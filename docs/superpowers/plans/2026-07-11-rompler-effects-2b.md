...
**Tests:** neutral exact-ish bypass — drive 0 all Db 0: output equals input within 1e-9 (preGain 1, tanh(x)≈x is NOT exact — tanh(x) ≠ x! So: neutral EQ with drive 0 means s = tanh(x); NOT a bypass. Instead assert the EQ path is neutral: drive 0, all Db 0 → output === tanh(input) computed by hand in the test, within 1e-12 — use a small-amplitude probe signal, e.g. amp 1e-5, so float32 storage quantization of tanh(x) itself stays under the budget, same trick as rotary's depth-0/mix-1 test); drive saturates: drive 1 on a 0.9-amplitude sine — peak(out) < peak(in) * 1.2 AND waveform differs (harmonics) (measured: preGain = 1 + 1*4 = 5, tanh(0.9*5) ≈ 0.9998, ratio ≈ 1.111 — the plan originally said "* 1.05" but that's tighter than the algorithm actually achieves; 1.111 still demonstrates hard saturation against the unsaturated ratio of preGain = 5, so the bound was widened to 1.2, not the algorithm); lowDb +12 boosts a 100 Hz sine RMS by ~3.5–4.2x vs neutral while a 5 kHz sine changes < 1.3x (shelf selectivity, generous bounds; measured ≈3.72x and ≈1.06x — holds as stated); highDb −12 attenuates 8 kHz ≥ 1.8x while 100 Hz changes < 1.3x (measured ≈2.03x at 8 kHz; the plan originally said "≥ 2.5x" but the one-pole shelf's slope is much shallower near Nyquist than the low shelf's is near DC for this coefficient formula — even 20 kHz only reaches ≈2.5x attenuation, right at the edge of the audible band, so the bound was corrected down to 1.8x at 8 kHz for a robust mid-band probe); reset determinism; validation bounds; twin ref: drive 0.4, low +3, mid −2, high +4, level −1, 440 sine amp 0.5, warmup 512, 8 L + 8 R.
